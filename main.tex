...
Se va a utilizar como referencia el documento de ``UTADEO - Ficha técnica servidores'' que se encuentra en la sección \ref{sec:anexos} de anexos. Este documento contiene las específicaciones técnicas de los equipos que se encuentran en el laboratorio de redes, el documento (según la \textit{metadata}) es de la fecha 4 de junio de 2025, tener en cuenta que pueden haber actualizaciones debido a las fechas de redacción.\footnote{ Si desea verificar la fecha que se afirma en este documento, puede visitar el repositorio de \textit{GitHub} del documento \href{https://github.com/SegmentationFaultUtadeo/PropuestaMaratonUtadeo/blob/main/Bibliografia/FichaTecnica.pdf}{\textbf{aquí}}, descargar el pdf e inspeccionar la \textit{metadata} del documento (comando \texttt{exiftool}\cite{exiftool}).} A su vez, se toma en cuenta los equipos y espacios que cuenta actualmente la universidad.
...
La inscripción va a ser para los voluntarios como para los equipos. En el caso de las personas que quieran ser ayudantes del evento, se diligencia un formulario individual. Para el caso de la inscripción de equipos se llenará un solo formulario por grupo.\footnote{ Se recomienda utilizar una instancia local de LimeSurvey\cite{limesurvey} para tener dominio de los datos y no depender de servicios de terceros.}

Los campos mínimos necesarios a incluir en la inscripción de los equipos son:

\begin{itemize}
  \item \textbf{Nombre del equipo:} Título que llevará el equipo en la competencia. Sin lenguaje ofensivo o inadecuado.
  \item \textbf{Información de cada integrante (x3):}
        \begin{itemize}
          \item \textbf{Nombre completo:} Nombres y apellidos.
          \item \textbf{Correo institucional:} Con dirección \texttt{@utadeo.edu.co}
          \item \textbf{Tipo de documento:} T.I, C.C, Pasaporte.
          \item \textbf{Número de documento:} NUIP del estudiante.
          \item \textbf{Programa académico:} Carrera de pregrado que está cursando el estudiante. Si es estudiante de doble carrera colocar la principal.
          \item \textbf{Semestre actual:} Semestre en el que se encuentra matriculado el estudiante.
          \item \textbf{¿Ha participado en competencias de programación competitiva?} Pregunta de sí o no.
        \end{itemize}
  \item \textbf{Comentarios adicionales:} Dudas, sugerencias o solicitudes del equipo.
  \item \textbf{Aceptación de términos y condiciones:} Pregunta de sí o no. Adjuntar las reglas de la competencia, uso de datos personales, comportamiento adecuado, etc.
\end{itemize}


Para aquellos individuos interesados en ser parte del equipo de voluntariado se deben llenar estos campos mínimos:

\begin{itemize}
  \item \textbf{Nombre completo:} Nombres y apellidos.
  \item \textbf{Correo electrónico:} Puede ser de cualquier dominio sin importar si es directamente afiliado a la universidad.
  \item \textbf{Número de contacto:} Número de celular, WhatsApp o cualquier otra red que sea de fácil interacción con el interesado.
  \item \textbf{¿Qué vínculo posee con la Universidad Jorge Tadeo Lozano?}
        \begin{itemize}[label=$\square$]
          \item  Estudiante actual.
          \item  Egresado/a.
          \item  Docente / Administrativo.
          \item  Externo/a.
          \item  Otro (especificar).
        \end{itemize}
  \item \textbf{Disponibilidad para (fecha del evento):} El interesado deberá marcar si está disponible para esa fecha o si puede durante un período de tiempo.
  \item \textbf{Confirmación para una reunión de inducción (virtual o presencial):} El interesado debe marcar si puede participar en un encuentro para explicar qué funciones se van a llevar durante el evento y para repartir roles.
  \item \textbf{Funciones que le gustaría colaborar:} Se pueden marcar más de una
        \begin{itemize}[label=$\square$]
          \item Registro de participantes.
          \item Apoyo logístico general (sillas, señalización, ubicaciones).
          \item Monitoreo de salas durante la competencia.
          \item Soporte técnico básico (conectividad, hardware).
          \item Glober@ (entregar globos a los equipos que resuelvan problemas).
          \item Cualquiera, donde se necesite.
        \end{itemize}
  \item \textbf{Compromiso:} El interesado acepta participar como voluntario y se compromete a cumplir con las tareas asignadas.
  \item \textbf{Comentarios adicionales:} Dudas, sugerencias o solicitudes del interesado.
\end{itemize}


Los formularios deben estar en todo momento cuando se esté en la fase de convocatoria y divulgación. Preferiblemente crear un grupo con todos los voluntarios para facilitar la comunicación entre todos los interesados.
...
Las normas para confirmar a los equipos en la competencia son simples y sencillas, se deben cumplir todas a continuación:

\begin{enumerate}
  \item El nombre del equipo no debe tener vulgaridades, insultos o cualquier forma de lenguaje soez.
  \item Los 3 miembros del equipo deben ser estudiantes (de cualquier carrera y semestre) matriculados en la universidad.
  \item Los datos proporcionados deben ser los correspondientes (correo, teléfono, tipo de documento\footnote{Para verificar el documento se pide el favor a la universidad ya que ellos poseen esos datos}).
\end{enumerate}


\subsection{Creación de equipos}

Todo lo que sigue se puede encontrar en \url{https://www.domjudge.org/docs/manual/8.2/config-basic.html}
